...
class VideoRecorder:
    def __init__(self, cfg_path="../cfgs/cfg.json", video_duration_minutes=1):
        ...
        self.fps = 30  # FPS objetivo para los videos
    ...
    def initialize_video_writers(self, first_frames):
        ...
        for camera_name, frame_data in first_frames.items():
            ...
            logger.info(f"Iniciado writer para {camera_name}: {output_path}")
    ...
    def close_current_writers(self):
        ...
        for camera_name, writer in self.writers.items():
            writer.release()
            logger.info(f"Video de {camera_name} finalizado con {self.frame_counts[camera_name]} frames")
        self.writers.clear()
        self.frame_counts.clear()

if __name__ == "__main__":
    # Crear instancia del grabador (5 minutos por video)
