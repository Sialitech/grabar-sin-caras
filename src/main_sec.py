...
logger = logging.getLogger("SequentialRecorder")

class SequentialVideoRecorder:
    def __init__(self, cfg_path="../cfgs/cfg.json", video_duration_minutes=1, fixed_fps=30):
        """
        Inicializa el grabador de video secuencial.
                ...
                while b"\xff\xd9" in buffer:
                    ...
                    if start != -1 and end > start:
                        ...
                        if frame is not None:
                            ...
                            # Verificar si se excedió la duración deseada
                            elapsed_time = time.time() - start_time
                            if elapsed_time >= self.video_duration:
                                logger.info(f"{camera_name}: Duración límite alcanzada.")
                                break

            if writer:
                writer.release()

            logger.info(f"{camera_name}: Grabación finalizada ({frame_count} frames, duración: {time.time() - start_time:.2f}s).")
        except Exception as e:
            logger.error(f"Error en la grabación de la cámara {camera_name}: {e}")

    def record_all(self):
        """
        Graba videos de todas las cámaras de manera secuencial.
        """
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_dir = Path("../files") / timestamp
        output_dir.mkdir(parents=True, exist_ok=True)

        logger.info("Iniciando grabación para todas las cámaras de manera secuencial...")

        for camera in self.cfg["cameras"]:
            self.record_camera(camera["name"], output_dir)

        logger.info("Grabación completada para todas las cámaras.")

if __name__ == "__main__":
    recorder = SequentialVideoRecorder(video_duration_minutes=1, fixed_fps=30)
    try:
        recorder.record_all()
